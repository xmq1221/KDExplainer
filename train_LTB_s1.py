...
from helper.util import str2bool, get_logger, preserve_memory, adjust_learning_rate
...
def get_dataloader(cfg: Dict[str, Any]):
    ...
    # print(len(train_loader), len(val_loader), num_classes)
    # logger.info(len(train_loader), len(val_loader), num_classes)
...
def train_epoch(
    ...
    for idx, (x, target) in enumerate(train_loader):
        ...
        # ===================forward=====================
        # logger.info(x.shape)
        logit = model(x, int(cfg["training"]["t"])/(epoch), True)
        # logit = model(x)
        # if loss_method == 'ce':
...
def train(
    ...
    for epoch in range(1, cfg["training"]["epochs"] + 1):
        ...
        global_lr, branch_lr = adjust_learning_rate(
                    cfg_lr_global=cfg["training"]["lr_global"],
                    cfg_lr_branch=cfg["training"]["lr_branch"],
                    optimizer=optimizer,
                    epoch_current=epoch,
                    epoch_sum=cfg["training"]["epochs"]
                    )

        logger.info("Start training epoch: %d, current lr: %.6f", 
                    epoch, lr_scheduler.get_last_lr()[0])
        logger.info("current global_lr: %.6f, current branch_lr: %.6f", 
                    global_lr, branch_lr)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default = '/home/yxy/kaecode3/LTB_kd/configs/cifar-10/seed-1029/LTB/train_LTB_ce.yml')
    parser.add_argument("--logdir", type=str, default = '/home/yxy/kaecode3/LTB_kd/log_LTB_s1')
    parser.add_argument("--file_name_cfg", type=str,default = 'train_LTB_ce.yml')
